...
struct Segment: Codable, Hashable {     // перегон между соседними станциями
    ...
    // Під час повітряної тривоги перегін не працює: "any" — при тривозі
    // будь-якого рівня (лівий берег червоної), "red" — лише при червоному
    // (Південний міст на зеленій). Правила КМДА з 10.09.2026; опційне поле.
    let alertStop: String?

    var alertStopRule: AlertStop? { alertStop.flatMap(AlertStop.init(rawValue:)) }

    // Единый ключ направленного перегона для сид-данных и калибровки.
    static func key(_ from: String, _ to: String) -> String { "\(from)>\(to)" }
}

enum AlertStop: String {
    case any, red
}

// Що тривога означає для конкретного маршруту. Рахується по перегонах, а не
// по станціях: «Видубичі» під землею, а міст за ними — ні.
enum AlertImpact: Equatable {
    case none
    case leftBank       // червона гілка: при будь-якій тривозі поїзди лише до «Арсенальної»
    case bridge         // зелена гілка: при червоному рівні міст не працює, при жовтому — їде
}

// Рівень тривоги, якщо джерело його повідомляє. Поточний фід каже лише
// «є / немає», тому nil — звичайний стан, і тексти мають це витримувати.
