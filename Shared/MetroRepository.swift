...
final class MetroRepository {
    ...
    // Доля интервала, которую закладываем на ожидание поезда после перехода.
    //
    // Половина интервала («пассажир приходит в случайный момент») выглядит
    // логично и на практике врёт. Во-первых, пассажир не приходит случайно:
    // он слышит поезд и ускоряется — полевая проверка 17.08.2026 дала ожидание
    // ровно ноль, потому что поезд стоял и человек забежал. Во-вторых, у ошибки
    // разная цена: если мы недооценили ожидание, отсчёт идёт ВПЕРЕДИ поезда и
    // предупреждение приходит рано — пассажир поднимает глаза и едет дальше.
    // Если переоценили — отсчёт отстаёт, и «наступна — ваша» приходит после
    // нужной станции. Второе — ровно тот провал, против которого весь продукт.
    // Поэтому осознанно смещаемся в раннюю сторону.
    private static let waitShareOfHeadway = 0.25

    func expectedWaitSeconds(lineId: String, forward: Bool, at date: Date) -> Int? {
        headwaySeconds(lineId: lineId, forward: forward, at: date)
            .map { Int(Double($0) * Self.waitShareOfHeadway) }
    }

    // Честное среднее — для ответа на вопрос «скільки триватиме поїздка»,
    // а не для відліку. Разница намеренная: планирование и предупреждение —
    // разные задачи, и оптимальная оценка у них разная.
    func averageWaitSeconds(lineId: String, forward: Bool, at date: Date) -> Int? {
