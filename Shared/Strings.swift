...
enum L10n {
    ...
    static var privacyPolicySections: [(title: String, body: String)] {
        [
            ("", tr("Оновлено: вересень 2026. Розробник: Владислав Домоцький, vladdomotsky@gmail.com.",
            ...
            (tr("Геолокація", "Location"),
             tr("Використовується лише коли застосунок відкритий (дозвіл «Під час використання»). На головному екрані — один раз, коли він відкривається, щоб підказати найближчу станцію («Станція поруч»). Під час поїздки — тільки на наземних ділянках метро, щоб уточнити розрахункове положення поїзда. В обох випадках координати обробляються на пристрої і не зберігаються. У режимі калібрування, який ви вмикаєте вручну, усі отримані координати (у тому числі приблизні, під землею) записуються у файл на пристрої разом із часом — це трек вашої поїздки. Нагадування «на станції», якщо ви його ввімкнете: зону станції відстежує сама iOS і показує сповіщення, коли ви поруч; застосунок вашого місця при цьому не бачить. У фоні геолокація не використовується. Без дозволу на геолокацію застосунок повністю функціональний.",
                "Used only while the app is open (“While Using” permission). On the main screen — once, when it opens, to suggest the nearest station (“Station nearby”). During a trip — only on above-ground metro sections, to refine the train’s estimated position. In both cases coordinates are processed on device and not stored. In the manually enabled Calibration mode, all received coordinates (including approximate ones underground) are written to a file on the device together with timestamps — a track of your ride. For the station reminder, if you turn it on, iOS itself watches the station area and shows the notification when you are near; the app never sees your location. Location is never used in the background. The app is fully functional without location permission.")),
