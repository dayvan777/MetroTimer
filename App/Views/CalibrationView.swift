...
struct CalibrationView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            // Экран можно закрыть свайпом, минуя «Готово». Без этого сессия
            // оставалась незакрытой: хвост буфера акселерометра не сбрасывался
            // на диск, CSV обрывался без строки «end», а акселерометр и GPS
            // продолжали писать до тех пор, пока модель не соберёт сборщик
            // мусора. Политика обещает запись только в режиме калибрування —
            // значит выход из него обязан её останавливать.
